...
class MJ_Controller(object):
    ...
    def __init__(self, model=None, simulation=None, viewer=None):
        ...
        if viewer is None:
            self.viewer = mujoco.viewer.launch_passive(self.model, self.data)  # Do NOT pass `data`
        else:
            self.viewer = viewer
        ...
        # self.ee_chain = Chain.from_urdf_file(path + "/MuJoCo_RL_UR5/UR5+gripper/ur5_gripper.urdf")
        ...
        for i in range(len(self.data.ctrl)):
            print(
                "Actuator ID: {}, Actuator Name: {}, Controlled Joint: {}, Control Range: {}".format(
                    i,
                    self.model.names[self.model.name_actuatoradr[i]],
                    self.actuators[i][3],
    ...
    def create_lists(self):
        ...
        self.current_target_joint_values = [
            self.controller_list[i].setpoint for i in range(6)
        ]
        ...
        self.actuators = []
        for i in range(6):
            item = [i, self.model.names[self.model.name_actuatoradr[i]]]
            item.append(self.model.actuator_trnid[i][0])
            item.append(self.model.names[self.model.name_jntadr[i]])
            item.append(self.controller_list[i])
            self.actuators.append(item)

    def actuate_joint_group(self, group, motor_values):
        try:
            assert group in self.groups.keys(), f"No group with name {group} exists!"
            assert len(motor_values) == len(self.groups[group]), (
                f"Invalid number of actuator values! Expected {len(self.groups[group])}, got {len(motor_values)}."
            )

            for i, v in enumerate(self.groups[group]):
                self.data.ctrl[v] = motor_values[i]  # ✅ Ensure correct indexing

        except Exception as e:
            print(e)
            print(f"Could not actuate requested joint group '{group}'.")
        ...
        try:
            ...
            if target is not None:
                assert len(target) == len(
                    self.groups[group]
                ), f"Mismatching target dimensions for group {group}!"
            ...
            while not self.reached_target:
                ...
                # We still want to actuate all motors towards their targets
                for j in range(len(self.data.ctrl)):
                ...
                if steps % 1000 == 0 and target is not None and not quiet:
                    print(
                        f"Moving group {group} to joint target! Max. delta: {max(deltas)}, Joint: {self.actuators[np.argmax(deltas)][3]}"
                    )
                ...
                ee_link_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "ee_link")
                ee_pos = self.data.xpos[ee_link_id].copy()  # Get a copy of the position
                temp = ee_pos - np.array([0, -0.005, 0.16])
                ...
                if max(deltas) < tolerance:
                    if target is not None and not quiet:
                        print(
                            colored(
                                f"Joint values for group {group} within requested tolerance! ({steps} steps)",
                                color="green",
                                attrs=["bold"],
                            )
                        )
                ...
                if steps > max_steps:
                    if not quiet:
                        print(
                            colored(
                                f"Max number of steps reached: {max_steps}",
                                color="red",
                                attrs=["bold"],
                            )
                        )
                        print("Deltas: ", deltas)
                    result = f"max. steps reached: {max_steps}"
                    break
            ...
            print(f"Error in move_group_to_joint_target: {e}")
            import traceback
            traceback.print_exc()
        ...
        for i in range(self.model.nbody):
            print(
                "Current position for body {}: {}".format(
                    self.model.body_id2name(i), self.data.body_xpos[i]
                )
            )
